...
public class AdminDao {
    ...
    private static final String NUMBER_RECIPE_QUERY = "SELECT COUNT(id) AS count_id FROM recipe WHERE admin_id = ?";
    ...
    public static int RecipeDao(int adminId) {
        try (PreparedStatement ps = DbUtil.getConnection().prepareStatement(NUMBER_RECIPE_QUERY)) {
            ps.setInt(1, adminId);
